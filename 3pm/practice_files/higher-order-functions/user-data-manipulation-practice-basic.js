...
const filterUsersByCity = city => users.filter(user => user.city === city);
// filter users whose messages array contains the message "Hello"
// const filterUsersWithPhrase = phrase => {
//   // filter the users array. for each user who whas messages look for some message that message includes phrase
...
const filterUsersWithPhrase = phrase => users.filter(user => user.messages.some(message => message.toLowerCase().includes(phrase.toLowerCase())));
...
module.exports = { users, sayHello, findUserByName, findUserById, filterUsersWithPhrase, increaseUserAge }